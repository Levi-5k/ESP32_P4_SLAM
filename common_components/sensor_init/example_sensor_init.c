...
void example_sensor_init(example_sensor_config_t *sensor_config, example_sensor_handle_t *out_sensor_handle)
{
    ...
    if (!cam) {
        ...
        ESP_LOGE(TAG, "Checked %d detection functions", 
                 (&__esp_cam_sensor_detect_fn_array_end - &__esp_cam_sensor_detect_fn_array_start));
        
        // Log the addresses we tried
        for (esp_cam_sensor_detect_fn_t *p = &__esp_cam_sensor_detect_fn_array_start; p < &__esp_cam_sensor_detect_fn_array_end; ++p) {
            ESP_LOGE(TAG, "Tried sensor at I2C address: 0x%02X, port: %d", p->sccb_addr, p->port);
        }
        return;
    }

    ESP_LOGI(TAG, "✅ Camera sensor detected successfully!");
    ESP_LOGI(TAG, "Detected sensor I2C address: 0x%02X", cam_config.sccb_handle ? 0x36 : 0x00);
    ...
    // Don't start streaming here - will be started after CSI controller is ready
    ESP_LOGI(TAG, "Sensor initialized, streaming will be started later");
    
    ESP_ERROR_CHECK(ret);
    out_sensor_handle->i2c_bus_handle = i2c_bus_handle;
    out_sensor_handle->sccb_handle = cam_config.sccb_handle;
    out_sensor_handle->sensor_device = cam;  // Store the sensor device handle
}

void example_sensor_deinit(example_sensor_handle_t sensor_handle)
{
    if (sensor_handle.sensor_device) {
        esp_cam_sensor_del_dev(sensor_handle.sensor_device);
    }
