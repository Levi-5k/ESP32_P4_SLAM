/*
 * Camera initialization for ESP32-P4-WIFI6 with OV5647 MIPI-CSI
...
// OV5647 MIPI-CSI Configuration for ESP32-P4-WIFI6 (using centralized pin config)
...
esp_err_t camera_init(void)
{
    ...
    ESP_LOGI(TAG, "✅ Camera controller created");
...
esp_err_t camera_set_exposure_mode(const char* mode)
{
    ...
    // For ESP32-P4, exposure control through sensor IOCTL is limited
    // Most exposure control is handled automatically by the OV5647 sensor
    if (s_sensor_handle.sensor_device) {
        ESP_LOGI(TAG, "✅ Exposure mode '%s' acknowledged (sensor handles automatically)", mode);
        return ESP_OK;
    } else {
...
esp_err_t camera_set_white_balance(const char* wb)
{
    ...
    // For ESP32-P4, white balance control through sensor IOCTL is limited
    // Most white balance control is handled automatically by the OV5647 sensor
    if (s_sensor_handle.sensor_device) {
        ESP_LOGI(TAG, "✅ White balance '%s' acknowledged (sensor handles automatically)", wb);
        return ESP_OK;
    } else {
